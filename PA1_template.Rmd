...
Calculate the total number of steps taken by each day `DailyTotalSteps` and create a histogram plot that is named `MeanTotalSteps`.
...
Create a histogram for total number of steps taken by each day using the new dataset. The plot is named `MeanTotalStepsXNa`.
...
Create a panel plot containing compareing average numbers of steps of each 5-minute invertal between weekdays and weekend days. The plot is called `MeanDailyInterval2`. It is very obvious that a higher peak happens earlier on weekdays and people are generally more active on weekends.
